...
- `timer_time == 0` 时每帧调用 `ActionOnTimer`；否则按 `SDL_GetTicks() / timer_time` 分频（SDL3 中 `SDL_GetTicks` 即返回 64 位毫秒时间戳）。
...
> 为什么还"不够快"？因为 SDL3 每次 `SDL_RenderPresent` 仍要交换整个缓冲区，脏区重绘主要省掉了"纹理创建 / 整幅绘制"的 CPU 开销。这在复杂场景下依然有明显的性能收益。
